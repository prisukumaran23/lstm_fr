...
###############################################################################
# Initialise neptune run and log params
###############################################################################
# Include project name and neptune api token to track model runs on neptune
run = neptune.init(
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            run["train_loss/batch"].log(cur_loss)
            run["train_ppl/batch"].log(math.exp(cur_loss))
            total_loss = 0
            start_time = time.time()
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        logging.info('-' * 89)
        run["valid_loss/batch"].log(val_loss)
        run["valid_ppl/batch"].log(math.exp(val_loss))
...
with open(args.save, 'rb') as f:
    model = torch.load(f)
    # Save the best model to neptune run
    run["model"].upload(args.save)
...
run["test_ppl"] = math.exp(test_loss)
run["config/model"] = args.model
run["config/criterion"] = criterion
run["config/dataset/path"] = args.data